        ...
        @if(count($carts) > 0)
            @foreach($carts as $cart)
                <div class="card d-flex flex-row mb-3" >
                    <a class="d-flex" href="/review/{{$cart->id}}">
                    ...
                    <div class="pl-2 d-flex flex-grow-1 min-width-zero">
                        <div class="card-body align-self-center d-flex flex-column flex-lg-row justify-content-between min-width-zero align-items-lg-center">
                            <a href="/review/{{$cart->id}}" class="w-40 w-sm-100">
                                <p class="list-item-heading mb-1 truncate">{{$cart->package->nama}}</p>
                            </a>
                            <p class="mb-1 text-muted text-small w-15 w-sm-100">{{$cart->address}}, {{ucwords(strtolower($cart->district['name']))}}</p>
                            <div class="w-15 w-sm-100">
                                @if($cart->respon != NULL)
                                    <span class="badge badge-pill badge-success">WO Sudah Merespon</span>
                                @else
                                    <span class="badge badge-pill badge-primary">Menunggu Respon WO</span>
                                @endif
                            </div>
                            <label class="mb-0 mt-3 mr-4">
            ...
            {{$carts->links()}}
        @else
            <p>Untuk sementara ini belum ada ulasan.</p>
        @endif
    </div>
</main>
@endsection
